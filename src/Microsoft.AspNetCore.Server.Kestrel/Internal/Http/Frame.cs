    ...
    public abstract partial class Frame : ConnectionContext, IFrameControl
    {
        ...
        protected RequestLineStatus TakeStartLine(SocketInput input)
        {
            ...
            try
            {
                ...
                {
                    if (scan.Seek(ref _vectorSpaces, end) == -1)
                    {
                ...
                var chFound = scan.Seek(ref _vectorSpaces, ref _vectorQuestionMarks, ref _vectorPercentages, end);
                if (chFound == -1)
                {
                ...
                {
                    needDecode = true;
                    chFound = scan.Seek(ref _vectorSpaces, ref _vectorQuestionMarks, end);
                    if (chFound == -1)
                    {
                        RejectRequest(RequestRejectionReason.MissingSpaceAfterTarget);
                    }
                ...
                scan.Take();
                begin = scan;
                if (scan.Seek(ref _vectorCRs, end) == -1)
                {
